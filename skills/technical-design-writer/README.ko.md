...
4. **완료 점검표로 자기 검토한다.** 어긋난 곳을 고치고, 근거를 대지 못하는 문장은 지운다. Mermaid 다이어그램은 모두 실제로 그려서 그림을 눈으로 본다. 문법이 맞아도 화살표가 잘리거나 글자가 겹치거나 한쪽으로만 길게 나올 수 있다.
...
| 낱말 하나에 뜻 하나 | 정해진 동작을 가리키는 낱말은 그 동작의 이름을 이미 품고 있어야 한다. 이름만 떼어 표의 열 이름이나 지표 이름으로 쓸 수 있어야 한다.<br>이름을 따로 만들어 붙여야 나오면 말할 때 쓰는 낱말이다.<br>보통 서술어는 이 시험을 받지 않는다. |
...
- 문장은 완전한 서술어로 끝낸다. 명사형 끝맺음과 행위자 없는 피동을 쓰지 않는다. `…로 이루어진다` → `…로 한다`, `…만이다` → `…뿐이다`
...
- [ ] 정해진 동작을 가리키는 낱말이, 표의 열 이름이나 지표 이름으로 홀로 쓸 수 있는 이름을 품고 있는가.

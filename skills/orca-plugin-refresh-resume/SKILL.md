...
- Each target session must have a lifecycle registry entry created by this plugin. Older sessions have no entry:
  ...
  Codex creates the entry only when a prompt starts, so a Codex session that never received a prompt blocks the plan
  until it receives one or is closed. Do not infer native session IDs from terminal previews or transcript files.
...
non-initiating sessions must be idle, registered, and free of unsent input. Claude Code background tasks and
session-scoped scheduled wakeups also prevent an idle verdict. Ask the user to run a new plan after resolving the
blocker.
...
Before each exit it checks the Orca `tui-idle` state and unsent input again. It sends `/exit` and waits until the agent
leaves its terminal, even when the terminal's shell stays open, then closes that tab and creates a new terminal. For
Codex it first waits until Codex releases the conversation lock, because its app-server can keep an exited
conversation open for about a minute. It resumes Codex with the model and reasoning effort last recorded for that
conversation. It does not force-kill a session or close a worktree.

A resumed Claude Code session counts as complete when `SessionStart` registers the target plugin version. Codex runs
`SessionStart` only when its next turn starts, so a resumed Codex session counts as complete when the new process holds
the conversation lock. Its receipt shows `plugin_registration: pending_next_turn` until the next prompt confirms it.
...
manual resume command in the private receipt for a session that the worker could not resume. If the error is
`codex_thread_still_open`, wait until Codex closes the conversation before running that command. If status reports
...
- A busy, unregistered, stale, duplicated, or ambiguous session, or one with unsent input, blocks the operation before
  plugin installation.
- For Codex, the worker reads the conversation lock and only the model and reasoning-effort fields of the conversation

...
violation inside the scope is reported.
    ...
    local-path               an absolute filesystem path (including /tmp, Windows drives,
                             ~/ and file:// URLs), or an unresolved relative link to a supported
                             source-document type; http and https URLs are never reported
...
SOURCE_DOCUMENT_LINK_PATTERN = re.compile(
    r"\.(?:md|markdown|html?|pdf|docx)(?:[?#][^\"'\s]*)?$",
    re.IGNORECASE,
)

REMOTE_LINK_PATTERN = re.compile(r"^(?:https?|mailto|tel):", re.IGNORECASE)
    ...
    def _check_attributes(self, tag, attrs):
        ...
        for name, value in values.items():
            ...
            if name in LINK_ATTRIBUTES:
                ...
                is_attachment = ATTACHMENT_PATTERN.search(stripped) is not None
                if (
                    name == "href"
                    and not REMOTE_LINK_PATTERN.search(stripped)
                    and not is_attachment
                    and SOURCE_DOCUMENT_LINK_PATTERN.search(stripped)
                ):
                    self._add(
                        "local-path",
                        "{} points at an unresolved source document: {}".format(name, stripped),
                    )
